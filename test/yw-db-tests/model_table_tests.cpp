...
YW_TEST_FIXTURE(ModelTable)
   ...
   YesWorkflowDB ywdb{ false };
	...
	YW_TEST_SETUP(ModelTable)
	{
		...
		Expect::AreEqual(1, (user1 = ywdb.insert(UserRow{ auto_id, "user1" })));
		Expect::AreEqual(2, (user2 = ywdb.insert(UserRow{ auto_id, "user2" })));
		Expect::AreEqual(3, (user3 = ywdb.insert(UserRow{ auto_id, "user3" })));
...
YW_TEST_SET
    ...
    {		
		Assert::AreEqual(1, ywdb.insert(ModelRow{ auto_id, user1, "2017-06-22 10:52:00.000" }));
    }

    YW_TEST(ModelTable, InsertTwoRows_SecondGeneratedIdIs_2)
    {
		Expect::AreEqual(1, ywdb.insert(ModelRow{ auto_id, user1, "2017-06-22 10:52:00.000" }));
		
		Assert::AreEqual(2, ywdb.insert(ModelRow{ auto_id, user2, "2017-06-22 10:52:01.000" }));
    }

    YW_TEST(ModelTable, SelectModelById_RowExists) {
        
		Expect::AreEqual(1, ywdb.insert(ModelRow{ auto_id, user2, "2017-06-22 10:52:00.000" }));
		Expect::AreEqual(2, ywdb.insert(ModelRow{ auto_id, user3, "2017-06-22 10:52:01.000" }));
