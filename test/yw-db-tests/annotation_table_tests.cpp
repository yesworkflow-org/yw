...
YW_TEST_FIXTURE(AnnotationTable)
	...
	YW_TEST_SETUP(AnnotationTable)
	{
		...
		Expect::AreEqual(8, ywdb.insert(SourceRow{ (source8 = 8), file5, "C" }));
...
YW_TEST_SET
    ...
    {
		Assert::AreEqual(1, ywdb.insert(AnnotationRow{ auto_id, extraction17, Tag::BEGIN, null_id, line19, 1, 0, 11, "@begin", "block" }));
    }

    YW_TEST(AnnotationTable, InsertingQualifyingAnnotationYieldsGeneratedId2)
	{
		long annotation1;
		Assert::AreEqual(1, (annotation1 = ywdb.insert(AnnotationRow{ auto_id, extraction17, Tag::BEGIN, null_id, line19, 1, 0, 11, "@begin", "block" } )));
		Assert::AreEqual(2, ywdb.insert(AnnotationRow{ auto_id, extraction17, Tag::IN, annotation1, line25, 1, 0, 7, "@in", "port" }));
	}
	...
	YW_TEST(AnnotationTable, SelectTopLevelAnnotationsReturnsOneRowWhenJustOneBeginAnnotation) {
		ywdb.insert(AnnotationRow{ auto_id, extraction17, Tag::BEGIN, null_id, line19, 1, 0, 11, "@begin", "block" });
		auto selectedAnnotations = ywdb.selectTopLevelAnnotations();

		Assert::AreEqual(1, selectedAnnotations.size());
		Assert::AreEqual(AnnotationRow{ 1, extraction17, Tag::BEGIN, null_id, line19, 1, 0, 11, "@begin", "block" }, selectedAnnotations[0]);
	}

	YW_TEST(AnnotationTable, SelectTopLevelAnnotationsReturnsTwoRowsWhenJustTwoBeginAnnotations) {
		ywdb.insert(AnnotationRow{ auto_id, extraction17, Tag::BEGIN, null_id, line19, 1, 0, 11, "@begin", "block" });
		ywdb.insert(AnnotationRow{ auto_id, extraction17, Tag::BEGIN, null_id, line119, 1, 0, 18, "@begin", "second block" });
		auto selectedAnnotations = ywdb.selectTopLevelAnnotations();

		Assert::AreEqual(2, selectedAnnotations.size());
		Assert::AreEqual(AnnotationRow{ 1, extraction17, Tag::BEGIN, null_id, line19, 1, 0, 11, "@begin", "block" }, selectedAnnotations[0]);
		Assert::AreEqual(AnnotationRow{ 2, extraction17, Tag::BEGIN, null_id, line119, 1, 0, 18, "@begin", "second block" }, selectedAnnotations[1]);
	}

	YW_TEST(AnnotationTable, SelectTopLevelAnnotationsReturnsTwoRowsWhenTwoTreesInserted) {

		auto firstRoot = AnnotationRow{ 1, extraction17, Tag::BEGIN, null_id, line19, 1, 0, 11, "@begin", "block" };
		ywdb.insert(firstRoot);
		ywdb.insert(AnnotationRow{ 2, extraction17, Tag::IN, firstRoot.id, line25, 1, 0, 8, "@in", "port" });
		ywdb.insert(AnnotationRow{ 3, extraction17, Tag::END, firstRoot.id, line77, 1, 0, 10, "@end", "block" });

		auto secondRoot = AnnotationRow{ 4, extraction17, Tag::BEGIN, null_id, line119, 1, 0, 18, "@begin", "second block" };
		ywdb.insert(secondRoot);
		ywdb.insert(AnnotationRow{ auto_id, extraction17, Tag::IN, secondRoot.id, line125, 1, 0, 15, "@in", "second port" });
		ywdb.insert(AnnotationRow{ auto_id, extraction17, Tag::END, secondRoot.id, line177, 1, 0, 17, "@end", "second block" });

		auto selectedAnnotations = ywdb.selectTopLevelAnnotations();

		Assert::AreEqual(2, selectedAnnotations.size());
		Assert::AreEqual(AnnotationRow{ 1, extraction17, Tag::BEGIN, null_id, line19, 1, 0, 11, "@begin", "block" }, selectedAnnotations[0]);
		Assert::AreEqual(AnnotationRow{ 4, extraction17, Tag::BEGIN, null_id, line119, 1, 0, 18, "@begin", "second block" }, selectedAnnotations[1]);
	}

	YW_TEST(AnnotationTable, SelectAnnotationTreeReturnsOneRowWhenRootAnnotationHasNoChildren) {
		auto annotation1 = AnnotationRow{ auto_id, extraction17, Tag::BEGIN, null_id, line19, 1, 0, 11, "@begin", "block" };
	...
	YW_TEST(AnnotationTable, SelectAnnotationTreeReturnsAllRowWhenOnlyRootAndChildrenInserted) {
		...
		auto annotation2 = AnnotationRow{ auto_id, extraction17, Tag::IN, rootAnnotationId, line25, 1, 0, 8, "@in", "port" };
		auto annotation3 = AnnotationRow{ auto_id, extraction17, Tag::END, rootAnnotationId, line77, 1, 0, 10, "@end", "block" };
		...
		Assert::AreEqual(AnnotationRow{ 1, extraction17, Tag::BEGIN, null_id, line19, 1, 0, 11, "@begin", "block" }, selectedAnnotations[0]);
		Assert::AreEqual(AnnotationRow{ 2, extraction17, Tag::IN, 1, line25, 1, 0, 8, "@in", "port" }, selectedAnnotations[1]);
		Assert::AreEqual(AnnotationRow{ 3, extraction17, Tag::END, 1, line77, 1, 0, 10, "@end", "block" }, selectedAnnotations[2]);
	}
	...
	YW_TEST(AnnotationTable, SelectAnnotationTreeReturnsTwoDifferentTreesDependingOnRoot) {
		...
		auto annotation2 = AnnotationRow{ auto_id, extraction17, Tag::IN, firstRootAnnotationId, line25, 1, 0, 8, "@in", "port" };
		auto annotation3 = AnnotationRow{ auto_id, extraction17, Tag::END, firstRootAnnotationId, line77, 1, 0, 10, "@end", "block" };
		ywdb.insert(annotation2);
		ywdb.insert(annotation3);
		Expect::AreEqual(3, ywdb.getRowCount("annotation"));

		auto annotation4 = AnnotationRow{ auto_id, extraction17, Tag::BEGIN, null_id, line119, 1, 0, 18, "@begin", "second block" };
		long secondRootAnnotationId;
		Expect::AreEqual(4, (secondRootAnnotationId = ywdb.insert(annotation4)));
		auto annotation5 = AnnotationRow{ auto_id, extraction17, Tag::IN, secondRootAnnotationId, line125, 1, 0, 15, "@in", "second port" };
		auto annotation6 = AnnotationRow{ auto_id, extraction17, Tag::END, secondRootAnnotationId, line177, 1, 0, 17, "@end", "second block" };
		...
		Assert::AreEqual(AnnotationRow{ 1, extraction17, Tag::BEGIN, null_id, line19, 1, 0, 11, "@begin", "block" }, firstAnnotationTree[0]);
		Assert::AreEqual(AnnotationRow{ 2, extraction17, Tag::IN, 1, line25, 1, 0, 8, "@in", "port" }, firstAnnotationTree[1]);
		Assert::AreEqual(AnnotationRow{ 3, extraction17, Tag::END, 1, line77, 1, 0, 10, "@end", "block" }, firstAnnotationTree[2]);

		Assert::AreEqual(3, secondAnnotationTree.size());
		Assert::AreEqual(AnnotationRow{ 4, extraction17, Tag::BEGIN, null_id, line119, 1, 0, 18, "@begin", "second block" }, secondAnnotationTree[0]);
		Assert::AreEqual(AnnotationRow{ 5, extraction17, Tag::IN, 4, line125, 1, 0, 15, "@in", "second port" }, secondAnnotationTree[1]);
		Assert::AreEqual(AnnotationRow{ 6, extraction17, Tag::END, 4, line177, 1, 0, 17, "@end", "second block" }, secondAnnotationTree[2]);
	}
