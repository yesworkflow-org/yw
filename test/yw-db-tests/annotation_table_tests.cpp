...
YW_TEST_SET
    ...
    {
		Assert::AreEqual(1, ywdb.insert(AnnotationRow{ auto_id, null_id, 1, 0, 11, "@begin", "block" }));
    }

    YW_TEST(AnnotationTable, InsertingQualifyingAnnotationYieldsGeneratedId2)
	{
		long annotation1;
		Assert::AreEqual(1, (annotation1 = ywdb.insert(AnnotationRow{ auto_id, null_id, 1, 0, 11, "@begin", "block" } )));
		Assert::AreEqual(2, ywdb.insert(AnnotationRow{ auto_id, annotation1 , 2, 0, 7, "@in", "port" }));
	}
